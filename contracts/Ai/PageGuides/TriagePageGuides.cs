...
public static class TriagePageGuides
{
    public static readonly IReadOnlyList<PageGuide> Guides = new PageGuide[]
    {
        new("/control-centre", "Control Centre",
            ...
            + "valuation, ONE row per period (the Client pane's Valuation reports section, "
            + "2026-09-15): the period's live frozen statement when one exists — "
            + "list_valuation_snapshots, a non-superseded id, type ValuationReportSnapshot — "
            + "otherwise the live claim — get_valuation_context, its ValuationClaimId, type "
            + "ValuationClaim. Claim and statement read each other's mail, so the period's story "
            + "is whole from either page. The SALES pane (2026-09-15) is the prospect side: an "
            + "estimate enquiry forwarded to the projects mailbox is tagged to the sales LEAD it is "
            + "about — file_email_to_record type lead (list_leads / find_by_reference LD-#### find "
            + "it; a lead belongs to no project, so no project is needed) — or a new lead is raised "
            + "from the email with create_lead_from_message (Raise Lead in the pane's Actions: "
            + "lands Engaged / Inbound with the email tagged to it). Then the estimate is opened "
            + "on the lead's page (/sales/leads/{leadId}: create_estimate). Filing attachments to "
