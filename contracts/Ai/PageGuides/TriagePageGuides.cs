...
public static class TriagePageGuides
{
    public static readonly IReadOnlyList<PageGuide> Guides = new PageGuide[]
    {
        new("/control-centre", "Control Centre",
            ...
            + "away: stage record tags with stage_triage_tag and to-dos with stage_triage_todo "
            + "(nothing lands until the user presses Apply), open the New email composer with "
            + "open_modal compose_email and write into it with update_open_modal, read the open email "
            + "and everything already staged through get_current_context, and find records to stage "
