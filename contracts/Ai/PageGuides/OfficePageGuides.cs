...
public static class OfficePageGuides
{
    public static readonly IReadOnlyList<PageGuide> Guides = new PageGuide[]
    {
        new("/todos", "To-dos",
            ...
            + "only), title, assignee, due date and notes. EVERY to-do names a role: the assignee "
            + "picker starts empty and the author picks a real role or Miscellaneous (\"no role "
            + "owns this\", a conscious choice, never a default) before the item saves — so when "
            + "adding one for the user, ask who owns it, offering Miscellaneous by name, rather "
            + "than guessing. Clicking an item opens its detail page. "
        ...
        new("/sales/leads/{leadId}", "Lead",
            ...
            + "logged by hand plus every stage change, newest first. The Actions menu holds "
            + "\"Edit details\", \"Log activity\" (a touch on the timeline) and, for directors on a "
            + "lead that is not Won, \"Delete lead\" — removes the lead with its timeline, "
            + "estimates, proposals and imagine rounds, for a mistaken capture or a duplicate, no "
            + "undo. A Won lead links to its project. Over the connector: get_lead reads it "
            ...
            + "drawings and specs), get_lead lists estimates[] with estimateId and their "
            + "sections[] (the priced breakdown), create_estimate / update_estimate_details / "
            + "set_estimate_breakdown / move_estimate_status act on them, find_by_reference "
            + "resolves EST-####. Each row's reference and Open go to the estimate's own page "
            + "(the breakdown editor and the document text); PDF downloads the client-facing "
            + "estimate document. The Proposals panel is on the connector too: get_lead lists "
        ...
        new("/sales/leads/{leadId}/estimates/{estimateId}", "Estimate",
            "One estimate on a lead (2026-09-15): the priced breakdown in the shape of Jewel's "
            + "tender documents — sections (Preliminaries & preambles, Demolition & stripping out, "
            + "Structural steelwork, Joinery, Plumbing & heating, Electrical, Glazing…), each a "
            + "table of lines: cost code (from the cost-centre master, optional), description, "
            + "quantity, unit, unit price, the total computed as you type; a section can be marked "
            + "a provisional allowance (the document prints it in red). \"Save breakdown\" writes "
            + "the whole thing — what is on the page is what the estimate becomes — and the "
            + "estimate's total becomes the sum. Beside it, \"What the document says\": the "
            + "executive summary, build time and exclusions that print after the project page. "
            + "Actions: Download PDF (cover, project page with Jewel's contact block, executive "
            + "summary, breakdown chart, one itemised table per section with its total, total "
            + "project estimate value, contact page — client-facing, the internal notes never "
            + "print), Edit details…, Move status…. Over the connector: get_lead lists the "
            + "estimate with sections[]; set_estimate_breakdown (confirm-first, full-record, "
            + "costCode from list_cost_codes) prices it — from the enquiry's drawings "
            + "(read_email_attachment) and Jewel's rates, saying which figures are allowances; "
            + "update_estimate_details writes the executive summary, build time and exclusions; "
            + "move_estimate_status to Submitted once the price has gone to the prospect."),
