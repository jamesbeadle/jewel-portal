...
public static class CommercialPageGuides
{
    public static readonly IReadOnlyList<PageGuide> Guides = new PageGuide[]
    {
        ...
        new("/projects/{project}/valuation", "Valuation Report",
            ...
            + "edited while the claim is Draft; the Valuation Invoices, Snapshots and Correspondence "
            + "sections sit inline, and working-copy PDF/Excel exports are always available. Each "
            + "claim is a linkable record in the Control Centre (Client → Valuation reports — one row "
            + "per period: the live frozen statement when one exists, else the claim; mail tag "
            + "JPMS/VAL-{project}-{claim number}): the Correspondence section reads mail filed to "
            + "the claim AND to every statement frozen from it, and each snapshot reads the same; "
            + "rolling over starts the next claim with its own tag. The PDF prints the "
            + "statement at the level the client reads it — one row per area of the works, one per "
            + "variation order. The Excel workbook has a Summary tab (every contract line under its "
            + "area, variations as one row per order), a Detail tab that also expands each variation "
            + "order's lines, and a flat Raw data tab. The toolbar's "
