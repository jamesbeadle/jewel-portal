-- ============================================================================
-- Split By France V07: one net valuation line -> 2 trade/detail lines
-- Generated from split-variation-order-template.sql -- touches ONLY V07.
...
-- Run: sqlcmd -S sql-jpms-prod-54cf9e.database.windows.net -d jpms -U jpmsadmin -i scripts/splits/split-byfrance-v07.sql -b
-- ============================================================================
SET XACT_ABORT ON;
SET NOCOUNT ON;
BEGIN TRAN;

DECLARE @OldLineId   nvarchar(64)  = N'bf-vo-v07';
DECLARE @ExpectedNet decimal(18,4) = 1360.0000;

IF NOT EXISTS (SELECT 1 FROM dbo.ValuationLineItems WHERE ValuationLineItemId = @OldLineId)
BEGIN
    PRINT 'V07: old line already gone -- split appears to have run. Nothing to do.';
    COMMIT TRAN;
    RETURN;
END;
...
IF @Net <> @ExpectedNet
    THROW 50001, 'V07: line net differs from expected -- revalued since this script was written. Re-check before splitting.', 1;
...
INSERT INTO @NewLines VALUES
    (1, N'bf-vo-v07a', N'bf-cl18-vo-v07a', N'Site Supervision 3 days - Sept 24', 0, N'PRELIMS-SMG', 500.0000, 500.00),
    (2, N'bf-vo-v07b', N'bf-cl18-vo-v07b', N'Temporary toilet & health, safety and welfare - Sept 24', 0, N'PRELIMS-WEL', 860.0000, 860.00);

IF (SELECT SUM(Amount) FROM @NewLines) <> @Net
    THROW 50002, 'V07: new line amounts do not sum to the old line''s net.', 1;

IF EXISTS (SELECT 1 FROM dbo.ValuationLineItems
           WHERE ValuationLineItemId IN (SELECT LineId FROM @NewLines))
    THROW 50003, 'V07: one of the new line ids already exists.', 1;

IF (SELECT COUNT(DISTINCT ValuationClaimId) FROM dbo.ClaimLines
    WHERE ValuationLineItemId = @OldLineId) > 1
    THROW 50004, 'V07: more than one claim references this line -- extend the script before running.', 1;
...
IF (SELECT SUM(ClaimedNow) FROM @NewLines) <> @ClaimedTotal
    THROW 50005, 'V07: per-line claimed values do not sum to what is currently claimed in the DB (claim % edited since?). Re-check.', 1;
