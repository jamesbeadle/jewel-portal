-- ============================================================================
-- Split By France V35: one net valuation line -> 2 trade/detail lines
-- Generated from split-variation-order-template.sql -- touches ONLY V35.
...
-- Run: sqlcmd -S sql-jpms-prod-54cf9e.database.windows.net -d jpms -U jpmsadmin -i scripts/splits/split-byfrance-v35.sql -b
-- ============================================================================
SET XACT_ABORT ON;
SET NOCOUNT ON;
BEGIN TRAN;

DECLARE @OldLineId   nvarchar(64)  = N'bf-vo-v35';
DECLARE @ExpectedNet decimal(18,4) = 8790.0000;

IF NOT EXISTS (SELECT 1 FROM dbo.ValuationLineItems WHERE ValuationLineItemId = @OldLineId)
BEGIN
    PRINT 'V35: old line already gone -- split appears to have run. Nothing to do.';
    COMMIT TRAN;
    RETURN;
END;
...
IF @Net <> @ExpectedNet
    THROW 50001, 'V35: line net differs from expected -- revalued since this script was written. Re-check before splitting.', 1;
...
INSERT INTO @NewLines VALUES
    (1, N'bf-vo-v35a', N'bf-cl18-vo-v35a', N'Site Supervision - EOT-03', 0, N'PRELIMS-SMG', 7500.0000, 7500.00),
    (2, N'bf-vo-v35b', N'bf-cl18-vo-v35b', N'Temporary toilet & health, safety and welfare - EOT-03', 0, N'PRELIMS-WEL', 1290.0000, 1290.00);

IF (SELECT SUM(Amount) FROM @NewLines) <> @Net
    THROW 50002, 'V35: new line amounts do not sum to the old line''s net.', 1;

IF EXISTS (SELECT 1 FROM dbo.ValuationLineItems
           WHERE ValuationLineItemId IN (SELECT LineId FROM @NewLines))
    THROW 50003, 'V35: one of the new line ids already exists.', 1;

IF (SELECT COUNT(DISTINCT ValuationClaimId) FROM dbo.ClaimLines
    WHERE ValuationLineItemId = @OldLineId) > 1
    THROW 50004, 'V35: more than one claim references this line -- extend the script before running.', 1;
...
IF (SELECT SUM(ClaimedNow) FROM @NewLines) <> @ClaimedTotal
    THROW 50005, 'V35: per-line claimed values do not sum to what is currently claimed in the DB (claim % edited since?). Re-check.', 1;
