-- ============================================================================
-- Split By France V36: one net valuation line -> 2 trade/detail lines
-- Generated from split-variation-order-template.sql -- touches ONLY V36.
...
-- Run: sqlcmd -S sql-jpms-prod-54cf9e.database.windows.net -d jpms -U jpmsadmin -i scripts/splits/split-byfrance-v36.sql -b
-- ============================================================================
SET XACT_ABORT ON;
SET NOCOUNT ON;
BEGIN TRAN;

DECLARE @OldLineId   nvarchar(64)  = N'bf-vo-v36';
DECLARE @ExpectedNet decimal(18,4) = 17935.0000;

IF NOT EXISTS (SELECT 1 FROM dbo.ValuationLineItems WHERE ValuationLineItemId = @OldLineId)
BEGIN
    PRINT 'V36: old line already gone -- split appears to have run. Nothing to do.';
    COMMIT TRAN;
    RETURN;
END;
...
IF @Net <> @ExpectedNet
    THROW 50001, 'V36: line net differs from expected -- revalued since this script was written. Re-check before splitting.', 1;
...
INSERT INTO @NewLines VALUES
    (1, N'bf-vo-v36a', N'bf-cl18-vo-v36a', N'Annex kitchen/Utility - Howdens supply, worktops & cabinet installation (completed/omit group)', 0, N'SUP-KIT', 16320.0000, 16320.00),
    (2, N'bf-vo-v36b', N'bf-cl18-vo-v36b', N'Bertazzoni 90cm Ceiling Hood - supply & installation (specialist supply)', 0, N'SUP-APP', 1615.0000, 0.00);

IF (SELECT SUM(Amount) FROM @NewLines) <> @Net
    THROW 50002, 'V36: new line amounts do not sum to the old line''s net.', 1;

IF EXISTS (SELECT 1 FROM dbo.ValuationLineItems
           WHERE ValuationLineItemId IN (SELECT LineId FROM @NewLines))
    THROW 50003, 'V36: one of the new line ids already exists.', 1;

IF (SELECT COUNT(DISTINCT ValuationClaimId) FROM dbo.ClaimLines
    WHERE ValuationLineItemId = @OldLineId) > 1
    THROW 50004, 'V36: more than one claim references this line -- extend the script before running.', 1;
...
IF (SELECT SUM(ClaimedNow) FROM @NewLines) <> @ClaimedTotal
    THROW 50005, 'V36: per-line claimed values do not sum to what is currently claimed in the DB (claim % edited since?). Re-check.', 1;
