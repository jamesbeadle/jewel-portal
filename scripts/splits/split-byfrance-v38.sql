-- ============================================================================
-- Split By France V38: one net valuation line -> 2 trade/detail lines
-- Generated from split-variation-order-template.sql -- touches ONLY V38.
...
-- Run: sqlcmd -S sql-jpms-prod-54cf9e.database.windows.net -d jpms -U jpmsadmin -i scripts/splits/split-byfrance-v38.sql -b
-- ============================================================================
SET XACT_ABORT ON;
SET NOCOUNT ON;
BEGIN TRAN;

DECLARE @OldLineId   nvarchar(64)  = N'bf-vo-v38';
DECLARE @ExpectedNet decimal(18,4) = 23650.0000;

IF NOT EXISTS (SELECT 1 FROM dbo.ValuationLineItems WHERE ValuationLineItemId = @OldLineId)
BEGIN
    PRINT 'V38: old line already gone -- split appears to have run. Nothing to do.';
    COMMIT TRAN;
    RETURN;
END;
...
IF @Net <> @ExpectedNet
    THROW 50001, 'V38: line net differs from expected -- revalued since this script was written. Re-check before splitting.', 1;
...
INSERT INTO @NewLines VALUES
    (1, N'bf-vo-v38a', N'bf-cl18-vo-v38a', N'Fencing - Omit 6ft timber fencing; demolition of fencing & trees, close board fencing incl Perry St', 0, N'EXTW-FEN', 34650.0000, 34650.00),
    (2, N'bf-vo-v38b', N'bf-cl18-vo-v38b', N'Soft landscaping (planting and beds etc) - Provisional Sum omit', 2, N'EXTW-LND', -11000.0000, -11000.00);

IF (SELECT SUM(Amount) FROM @NewLines) <> @Net
    THROW 50002, 'V38: new line amounts do not sum to the old line''s net.', 1;

IF EXISTS (SELECT 1 FROM dbo.ValuationLineItems
           WHERE ValuationLineItemId IN (SELECT LineId FROM @NewLines))
    THROW 50003, 'V38: one of the new line ids already exists.', 1;

IF (SELECT COUNT(DISTINCT ValuationClaimId) FROM dbo.ClaimLines
    WHERE ValuationLineItemId = @OldLineId) > 1
    THROW 50004, 'V38: more than one claim references this line -- extend the script before running.', 1;
...
IF (SELECT SUM(ClaimedNow) FROM @NewLines) <> @ClaimedTotal
    THROW 50005, 'V38: per-line claimed values do not sum to what is currently claimed in the DB (claim % edited since?). Re-check.', 1;
