-- ============================================================================
-- Split By France V42: one net valuation line -> 2 trade/detail lines
-- Generated from split-variation-order-template.sql -- touches ONLY V42.
...
-- Run: sqlcmd -S sql-jpms-prod-54cf9e.database.windows.net -d jpms -U jpmsadmin -i scripts/splits/split-byfrance-v42.sql -b
-- ============================================================================
SET XACT_ABORT ON;
SET NOCOUNT ON;
BEGIN TRAN;

DECLARE @OldLineId   nvarchar(64)  = N'bf-vo-v42';
DECLARE @ExpectedNet decimal(18,4) = -14577.0000;

IF NOT EXISTS (SELECT 1 FROM dbo.ValuationLineItems WHERE ValuationLineItemId = @OldLineId)
BEGIN
    PRINT 'V42: old line already gone -- split appears to have run. Nothing to do.';
    COMMIT TRAN;
    RETURN;
END;
...
IF @Net <> @ExpectedNet
    THROW 50001, 'V42: line net differs from expected -- revalued since this script was written. Re-check before splitting.', 1;
...
INSERT INTO @NewLines VALUES
    (1, N'bf-vo-v42a', N'bf-cl18-vo-v42a', N'Tile Supply Only - As per Thompson & Leigh Quote 27/11/25 - NOW REVISED IN V69 WITH OMIT AND ADD BACK', 0, N'SUP-TIL', 14435.0000, 7217.50),
    (2, N'bf-vo-v42b', N'bf-cl18-vo-v42b', N'Wall & floor tiling, splashbacks & shower room omits (Install now V76 / Omit V66)', 2, N'TIL-STD', -29012.0000, -28580.00);

IF (SELECT SUM(Amount) FROM @NewLines) <> @Net
    THROW 50002, 'V42: new line amounts do not sum to the old line''s net.', 1;

IF EXISTS (SELECT 1 FROM dbo.ValuationLineItems
           WHERE ValuationLineItemId IN (SELECT LineId FROM @NewLines))
    THROW 50003, 'V42: one of the new line ids already exists.', 1;

IF (SELECT COUNT(DISTINCT ValuationClaimId) FROM dbo.ClaimLines
    WHERE ValuationLineItemId = @OldLineId) > 1
    THROW 50004, 'V42: more than one claim references this line -- extend the script before running.', 1;
...
IF (SELECT SUM(ClaimedNow) FROM @NewLines) <> @ClaimedTotal
    THROW 50005, 'V42: per-line claimed values do not sum to what is currently claimed in the DB (claim % edited since?). Re-check.', 1;
