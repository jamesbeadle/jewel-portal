...
public static partial class ValuationStatementRenderer
{
    ...
    private static void AddHeaderBand(Section section, ValuationStatementDocument document)
    {
        var snapshot = document.Statement.Claim;
        var statement = document.Statement;
        var facts = new List<HeaderFact>
        {
            new(statement.Label.ToUpperInvariant()),
            new(document.IsDraft
                ? $"Prepared  {DateAndTime(statement.AsAt)}"
                : $"Locked  {DateAndTime(statement.AsAt)}")
        };
    ...
    private static void AddDetailsGrid(Section section, ValuationStatementDocument document)
    {
        var snapshot = document.Statement.Claim;
        var statement = document.Statement;
        ...
        AddGridRow(table,
            ...
            document.IsDraft ? "Prepared" : "Locked", DateAndTime(statement.AsAt));
        AddGridRow(table,
    ...
    /// </summary>
    private static void AddMovementLegend(Section section, ValuationStatementDocument document)
    {
    ...
    private static void AddElementGroup(
        Section section, ValuationStatementDocument document, ValuationReportBillColumns columns,
        string title, ValuationElementType elementType)
    {
        ...
        var groups = ValuationReportBillRows.GroupsFor(document.Statement.Lines, elementType,
    ...
    private static void AddSummary(Section section, ValuationStatement detail)
    {
        var snapshot = detail.Claim;
    ...
    private static void AddClosingNote(Section section, ValuationStatementDocument document)
    {
        var snapshot = document.Statement.Claim;
        var statement = document.Statement;
        ...
        var note = section.AddParagraph(document.IsDraft
            ? "All figures are net of VAT. This is a WORKING COPY of the live valuation report as it stood "
              + $"when prepared on {Date(statement.AsAt)}: an internal review print of the claim as the report "
              + "currently shows it, not a statement issued to anyone. The statement issued for a claim is the "
              + "claim as locked, behind its valuation invoice on the Valuation Report tab. Contract, "
            ...
            : "All figures are net of VAT. This statement is a frozen record of the valuation report exactly "
              + $"as it stood when the claim was locked on {Date(statement.AsAt)}; work recorded since is "
              + "not reflected here. Contract, provisional and contingency items are shown individually under "
              + "their area headings; each approved variation shows as one consolidated row per order, and "
              + "orders not yet approved are not listed. Declined and to-be-confirmed items are not priced "
              + "into any total. If anything on this statement doesn't "
    ...
    private static void AddFooter(Section section, ValuationStatementDocument document)
    {
        var snapshot = document.Statement.Claim;
        var statement = document.Statement;
        HouseFooter(section, document.IsDraft
            ? $"Prepared {DateAndTime(statement.AsAt)} · working copy of the live report"
            : $"Locked {DateAndTime(statement.AsAt)} · issued statement from the JPMS register");
    }
}
