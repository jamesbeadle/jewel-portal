...
/// in the modal (ValuationSnapshotEmailModal), which meant the portal held two spellings of one
...
/// </summary>
public sealed partial class ValuationSnapshotEmailComposer : IComposesRecordEmail
{
    private readonly ValuationReportSnapshotPdfBuilder builder;
    private readonly JpmsContext context;

    public ValuationSnapshotEmailComposer(ValuationReportSnapshotPdfBuilder builder, JpmsContext context)
    {
        this.builder = builder;
        this.context = context;
    }

    public RecordType Record => RecordType.ValuationReportSnapshot;

    public RoleSet RolesThatMaySend =>
        SendValuationReportSnapshotEmailAuthorisation.RolesThatMayEmailSnapshots;
    ...
    public async Task<ComposedRecordEmail> ComposeAsync(RecordEmailDraft draft, CancellationToken cancellationToken)
    {
        ...
        var message = new MailboxDraftMessage(
            ...
            Subject: string.IsNullOrWhiteSpace(draft.Subject)
                ? DefaultSubject(pdf.Snapshot, projectName)
                : draft.Subject.Trim(),
            HtmlBody: string.IsNullOrWhiteSpace(draft.BodyHtml)
                ? DefaultBody(pdf.Snapshot, projectName)
                : draft.BodyHtml,
        ...
        return new ComposedRecordEmail(
            pdf.Snapshot.Label, pdf.ProjectId, message, Array.Empty<string>());
    }
