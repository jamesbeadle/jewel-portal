...
public sealed class ListMyTodoItemsEndpoint
{
    ...
    // Any internal user may read the items assigned to a role THEY hold — the role list is stamped
    // from the session here, never taken from the request, so this can't be used to browse another
    // role's list. The browse-everything read is ListAllTodoItems, gated separately.
