...
/// fileUploads), under one folder per name and kind. The folder is created with its first form.
/// </summary>
internal static class FormFolderFiling
{
    ...
    public static async Task<FormFolderEntity> FolderForAsync(
        JpmsContext context, FormDefinition form, string filingName, DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        var folder = await context.FormFolders.FirstOrDefaultAsync(
            row => row.NormalizedName == normalised && row.Kind == (int)form.FilingKind, cancellationToken);
        if (folder is null)
        {
            folder = NewFolder(name, normalised, form.FilingKind, now);
    ...
    private static FormFolderEntity NewFolder(string name, string normalised, FormFilingKind kind, DateTimeOffset now) => new()
    {
