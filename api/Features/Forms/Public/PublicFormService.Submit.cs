...
public sealed partial class PublicFormService
{
    ...
    public async Task<PublicFormReceipt> SubmitAsync(
        string companyCode, string slug, PublicFormSubmission posted, string clientHash, CancellationToken cancellationToken)
    {
        ...
        var link = await LinkOnSendingAsync(company, form.Slug, posted, cancellationToken);
        ...
        var uploads = await PostedUploadsAsync(posted, form.Slug, company, cancellationToken);
        ...
        var submission = await RecordAsync(form, company, answers, uploads, link, posted.SessionId, clientHash, cancellationToken);
        await AnnounceAsync(form, company, submission, answers, uploads);
        return new PublicFormReceipt(submission.FormSubmissionId, submission.IsVerifiedLink);
    }

    private async Task<ResolvedLink?> LinkOnSendingAsync(
        JewelCompany company, string formSlug, PublicFormSubmission posted, CancellationToken cancellationToken)
    {
        var hasALink = !string.IsNullOrEmpty(posted.InviteToken) || !string.IsNullOrEmpty(posted.PackToken);
        if (!hasALink) return null;
        var link = await ResolveAsync(company, formSlug, posted.InviteToken, posted.PackToken, cancellationToken);
        var particulars = JewelCompanies.For(company);
        var used = FormWording.DeadLink(FormLinkProblem.Used, particulars.Phone);
    ...
    private async Task<List<FormUploadEntity>> PostedUploadsAsync(
        PublicFormSubmission posted, string formSlug, JewelCompany company, CancellationToken cancellationToken)
    {
        ...
        return await context.FormUploads
            .Where(row => postedIds.Contains(row.FormUploadId) && row.SessionId == posted.SessionId && row.FormSlug == formSlug
                && row.Company == (int)company && row.FormSubmissionId == null && row.DeletedAt == null)
    ...
    private async Task<FormSubmissionEntity> RecordAsync(
        FormDefinition form, JewelCompany company, Dictionary<string, string> answers, List<FormUploadEntity> uploads,
        ResolvedLink? link, string sessionId, string clientHash, CancellationToken cancellationToken)
    {
        ...
        var folder = await FormFolderFiling.FolderForAsync(context, form, company, filingName, now, cancellationToken);
        var submission = PublicFormSubmissions.New(form, company, answers, link, folder, sessionId, clientHash, now);
