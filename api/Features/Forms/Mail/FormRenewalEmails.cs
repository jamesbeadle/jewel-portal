...
internal static class FormRenewalEmails
{
    public static FormEmail ForInsurance(
        string contactName, string email, string cover, DateOnly expiresOn, string link, DateTimeOffset linkExpiresAt)
    {
        ...
        return Chase(contactName, email, $"Your {cover.ToLowerInvariant()} renewal", ask, "Send the renewal", link, linkExpiresAt);
    }

    public static FormEmail ForTraining(
        string personName, string email, string course, DateOnly expiresOn, string link, DateTimeOffset linkExpiresAt)
    {
        ...
        return Chase(personName, email, $"Your {course} renewal", ask, "Send the new certificate", link, linkExpiresAt);
    }

    private static FormEmail Chase(
        string personName, string email, string subject, string ask, string buttonLabel, string link, DateTimeOffset linkExpiresAt)
    {
        ...
        var html = Wrap(Paragraph(Hello(personName)) + Paragraph(Encode(ask)) + Button(link, buttonLabel) + SmallPrint(smallPrint));
        var text = $"Hello {FirstName(personName)}\n\n{ask}\n\n{link}\n\nThe link works until {Day(linkExpiresAt)} and can only be used once."
            + $"\n\n{FootLine}";
        return new FormEmail(new[] { email }, $"{subject} - {JewelBespokeBuild.LegalName}", html, text);
    }
}
