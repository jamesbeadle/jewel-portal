...
/// <summary>
/// The look every forms email shares, carried from the dashboard: the company's name as a heading,
/// the body, and the company's registered particulars at the foot. The email a stranger receives
...
internal static class FormEmailFrame
{
    ...
    public static string Accent(JewelCompany company) =>
        company == JewelCompany.JewelPropertyServe ? "#135EAA" : "#101111";

    public static string Wrap(JewelCompany company, string body)
    {
        var particulars = JewelCompanies.For(company);
        return "<div style=\"font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#2F2F2F;line-height:1.55;max-width:560px\">"
            + $"<p style=\"font-weight:700;color:{Accent(company)};letter-spacing:.4px;margin:0 0 18px\">{Encode(particulars.ShortName.ToUpperInvariant())}</p>"
            + body
            + $"<p style=\"margin:14px 0 0;font-size:12px;color:#9AA3AF\">{Encode(FootLine(particulars))}</p></div>";
    }
    ...
    public static string Button(JewelCompany company, string link, string label) =>
        $"<p style=\"margin:18px 0\"><a href=\"{Encode(link)}\" style=\"display:inline-block;padding:13px 22px;background:{Accent(company)};"
    ...
    public static string FootLine(JewelCompanyParticulars particulars) =>
        string.Join(" · ", new[] { particulars.LegalName, particulars.RegisteredOffice, particulars.Phone }
