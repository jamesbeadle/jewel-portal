...
/// architect files and changes instructions only on the projects that name their practice
...
internal static class ArchitectInstructionScope
{
        ...
        JpmsContext context, SignedInUser user, string instructionId, CancellationToken cancellationToken)
    {
        ...
        var architectId = ArchitectScope.OwnArchitectId(user);
        if (architectId is null) return false;
        return await ArchitectProjects.OwnsInstructionAsync(context, architectId, instructionId, cancellationToken);
    }
}
