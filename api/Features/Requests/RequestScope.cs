...
/// role is the whole answer. An architect is confined to the projects their login was given
/// (ArchitectProjects), a client to its own (ClientProjects); a subcontractor may raise a request
/// only on a project they hold an issued work order for (SubcontractorProjects). An external login
/// with no link reaches nothing.
/// </summary>
internal static class RequestScope
{
        ...
        JpmsContext context, SignedInUser user, string requestId, CancellationToken cancellationToken)
    {
        ...
        var architectLogin = ArchitectScope.OwnArchitectLogin(user);
        if (architectLogin is not null)
            return await ArchitectProjects.OwnsRequestAsync(context, architectLogin, requestId, cancellationToken);
        var clientId = ClientScope.OwnClientId(user);
        if (clientId is null) return false;
        return await ClientProjects.OwnsRequestAsync(context, clientId, requestId, cancellationToken);
    }
}
