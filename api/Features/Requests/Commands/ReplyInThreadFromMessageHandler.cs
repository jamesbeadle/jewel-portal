...
/// <summary>
/// Triage "Reply in thread": create a General request from the email (delegating to the same
/// handler as "Create new → Request", so the email is tagged to the new request first, verified,
/// and the request is auto-numbered REQ-####), then stage an Outlook reply draft on the email in
/// the projects mailbox. Graph's createReplyAll keeps the draft in the original conversation —
/// "RE:" subject, thread headers, quoted history, original recipients — and the draft carries the
/// request's workflow category so the sent copy and its replies group under the request in triage.
/// Unlike <see cref="PrepareRequestReplyDraftHandler"/> no document is attached and the cover note
/// is empty: the triager writes the reply themselves and sends it from the mailbox — code never
/// sends. If the draft can't be staged, the just-created request is rolled back (tag removed,
/// request deleted) so the email stays in the queue rather than being triaged without a reply.
/// </summary>
public sealed class ReplyInThreadFromMessageHandler : ICommandHandler<ReplyInThreadFromMessage, ReplyInThreadOutcome>
{
    ...
    {
        // Read the email first so the request's title/description are built from the mailbox's own
        // copy (the client only sends ids), and so a vanished email fails before anything is created.
        ...
        // The request records HOW it was triaged (a reply in the thread) and carries the email's
        // content as its opening description, so the request page reads sensibly on its own.
        var description = $"Replied to email in thread with \"{subject}\".\n\n{snapshot.BodyPreview}".Trim();
        ...
        // Stage the reply draft: no attachment, empty cover note (the triager writes the reply in
        // Outlook, above the quoted history Graph supplies), tagged so the sent copy groups under
        // the new request in triage.
