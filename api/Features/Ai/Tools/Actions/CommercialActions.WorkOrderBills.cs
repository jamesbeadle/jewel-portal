    ...
    private static IEnumerable<AiAction> WorkOrderBillActions() => new AiAction[]
    {
        new AiAction(
            ...
            Notes: "Read list_xero_ledger_lines with status Unallocated first: a bill's lines carry "
                ...
                + "approve, nothing to key. BySupplierOrders means the ladder ran out — the bill "
                + "names no order and its total is not what is left on any one order or unique "
                + "combination (a part bill, or amounts that fit more than one way) — so every "
                + "proposed slice is 0: NEVER invent the split; show the user the supplier's open "
                + "orders with their remaining values and take the figure per order from them, or "
                + "leave the bill for the page. Move money between the supplier's open orders only "
