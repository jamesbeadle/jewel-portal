    ...
    private static IEnumerable<AiAction> FilingActions() => new AiAction[]
    {
        ...
        new AiAction(
            ...
            Notes: "messageId is the mailbox message id as read_record_emails / get_mailbox_message "
                ...
                + "name the record (recordId via find_by_reference). A valuation email files to the "
                + "valuation claim — type ValuationClaim, the ValuationClaimId from list_valuations or "
                + "get_valuation_context: since 2026-09-18 the claim IS the statement, one row per period. "
                + "(Type ValuationReportSnapshot with a retired snapshot id still lands on its claim.) "
