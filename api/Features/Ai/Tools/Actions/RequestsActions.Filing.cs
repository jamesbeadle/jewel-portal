    ...
    private static IEnumerable<AiAction> FilingActions() => new AiAction[]
    {
        ...
        new AiAction(
            ...
            Notes: "messageId is the mailbox message id as read_record_emails / get_mailbox_message "
                ...
                + "name the record (recordId via find_by_reference). A valuation email files to ONE "
                + "row per period (2026-09-15): the period's live frozen statement when one exists — "
                + "type ValuationReportSnapshot, a non-superseded id from list_valuation_snapshots — "
                + "otherwise the live claim — type ValuationClaim, the ValuationClaimId from "
                + "get_valuation_context. Either row reads the whole period's mail, so a wrong pick "
                + "between the two is never lost; superseded statements are not filing targets. "
                + "scope is ThreadBehindAnchor "
                ...
                + "record types: CostCentre (Client or Subcontractor) and Defect (Subcontractor or "
                + "Supplier — omitted, the defect's own company decides). type Lead (recordId = the leadId, no projectId) "
