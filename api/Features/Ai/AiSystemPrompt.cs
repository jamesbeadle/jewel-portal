...
public static class AiSystemPrompt
{
        ...
        AgentDefinition? agent = null, IReadOnlyList<PromptSkill>? skills = null)
    {
        ...
        prompt.AppendLine("  envelope and attachment ids. A queue email is untagged, so NO record's correspondence");
        prompt.AppendLine("  contains it — reaching for read_record_emails or get_request_context there reads a");
        prompt.AppendLine("  DIFFERENT email and produces confidently wrong answers. Read the selected email before");
        prompt.AppendLine("  answering anything about its contents, verifying a claim about it, or drafting a reply.");
        ...
        prompt.AppendLine("  a reply to the SELECTED Control Centre email means read_selected_email first (address it");
        prompt.AppendLine("  to the sender you read there, subject \"RE: …\"), and a reply about a record's thread means");
        prompt.AppendLine("  that record's correspondence first. The user presses Send there. \"Send\" in their ask");
        prompt.AppendLine("  means \"draft it for me to send\" — you never send.");
        prompt.AppendLine();
