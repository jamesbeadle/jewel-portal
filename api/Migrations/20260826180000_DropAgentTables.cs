...
{
    /// <inheritdoc />
    // Retires the per-record agent framework's storage (2026-08-26). The tables have been empty
    // since 20260702130000_ClearRequestAgents deleted every row and AgentProvisioning was switched
    // off; the conversational assistant (AgentCatalogue + AiTurnRunner, docs/ai/05) replaced the
    // model entirely, and the code that read these tables is deleted in the same change. Purely
    // subtractive; apply AFTER (or with) the deploy — the outgoing code still queries RequestAgents
    // on request close, so dropping first would break closing until the deploy lands.
    [DbContext(typeof(JpmsContext))]
    [Migration("20260826180000_DropAgentTables")]
    public partial class DropAgentTables : Migration
    {
        /// <inheritdoc />
        ...
        /// <inheritdoc />
        // Mirrors 20260629120000_AddRequestAgents exactly, so rolling back restores the (empty)
        // schema the framework left behind.
