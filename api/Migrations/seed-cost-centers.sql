-- ============================================================================
-- Seed the GLOBAL cost-center master  (JBB Cost Code Master, trade-prefixed)
-- ----------------------------------------------------------------------------
-- One shared cost-center list used by every project's Financials tab, the
-- Valuation Report CODE column and the Xero cost-allocation queue. The master
-- is the JBB Cost Code Master workbook (v2.1, owner Nigel Reilly): 138 codes
-- across 25 parent trades, trade-prefixed (PRELIMS-HRD, ENABLE-DEM, ...).
-- Every code = one subcontractor / bid package. If a code is not in the
-- master it is NOT a cost code. (Xero's own numeric tracking options are a
-- legacy numbering; the allocation queue translates them via suggestions.)
--
...
-- For an EXISTING database use migrate-cost-centers-v2.sql.
--
