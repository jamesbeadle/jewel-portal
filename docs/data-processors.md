...
| **Azure Communication Services** | The emails the portal sends: invites and password resets (`api/Auth/AzureEmailInviteNotifier.cs`), the imagine journey's emails to prospects (`api/Features/Sales/Imagine/ImagineNotifier.cs`), from `DoNotReply@mail.jewelbb.co.uk` | Recipient address and display name, a single-use link, concept and proposal wording | EEA |
...
| Right | How |
|---|---|
| Access | Admin → Data protection reads the dossier (`GetPersonDossier`): every row that is the person and every column that names them |
