...
- Existing personas P01–P05 (Architect, QS, Subcontractor, Accountant, MD) map cleanly to a subset of the operational roles. The QS persona (P02) absorbs into the **Project & Commercial Lead** role for JBB-internal staff; the Accountant persona (P04) maps onto the **Finance Director (FD)** role; the MD persona (P05) maps onto **Directors**. New operational roles (Project & Commercial Lead, Office & Compliance Coordinator, Site Team, Brand & Content, Outsourced IT helpdesk) are added to the persona register at draft status.
...
| D3 | Operational roles named in the audit (Project & Commercial Lead, Office & Compliance Coordinator, Site Team, Brand & Content, Outsourced IT helpdesk) are added to `/docs/requirements/personas.md` at draft status. The five 2026-05-18 personas remain; the QS, Accountant and MD personas are re-cast as draft mappings onto the operational roles. | Nigel | 2026-05-20 |
| D4 | A draft Role × Workflow RBAC matrix is created at `/docs/requirements/permission-matrix.md`. The five 2026-05-18 personas plus the new operational roles form the columns; the twenty-one workflows form the rows. Matrix granularity is coarse (`O` = owner, `C` = contributor, `A` = approver, `R` = read, `—` = no access) until each workflow's journey deep-dive refines it. | Nigel | 2026-05-20 |
...
- [ ] Reconcile workflow 11 (Cashflow & Management Reporting) with the Accountant's primary-pain-point cashflow-forecast journey from 2026-05-18 — they describe the same need from different angles — **Nigel**, due before the next finance session
...
- [ ] Does the existing QS persona (P02) remain a separate role on JPMS, or fold into Project & Commercial Lead? Affects RBAC and the personas register.
...
- Updated `/docs/requirements/personas.md` — added P06 Project & Commercial Lead, P07 Office & Compliance Coordinator, P08 Site Team, P09 Brand & Content, P10 Finance Director (re-cast from P04 Accountant), P11 Directors (re-cast from P05 MD), P12 Outsourced IT Helpdesk. All draft status, all sourced from this meeting note.
