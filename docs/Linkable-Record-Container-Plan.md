...
Status: **PRs 1–4 implemented (static‑verified; CI build pending)** · Date: 2026‑06‑30 · Relates to: `docs/Bid-Package-Invite-and-Agent-System-Spec.md` (Part B, Phase 1)
...
- **PR 4 — whole‑thread tagging (done).** Linking now tags the email's **entire Inbox conversation** (Graph `conversationId`), not just the clicked message, so the record's tag carries the full thread context. **Discard/restore are thread‑aware too:** discarding tags the whole conversation as discarded; restoring removes it across the thread (`RecordThreadTagger.UntagThreadAsync` + `ListTaggedInboxIdsInConversationAsync`). `RecordThreadTagger` tags the anchor (verified) + conversation siblings (best‑effort); new graph queries `ListInboxConversationIdsByCategoryAsync` / `ListUntaggedInboxIdsInConversationAsync` back it. A catch‑up sweep (`SyncRecordThreadTags` command + `RecordThreadTagger.SyncThreadsAsync`) re‑tags replies that arrive later; it runs best‑effort when a request's conversation view loads (`ListRequestMessagesHandler`) and is exposed as an endpoint/`IIntakeQueue.SyncRecordThreadTagsAsync` for any record type to call on open. Scope is Inbox‑only (matches the reader); `conversationId`‑only (no header fallback), per the chosen options.
- **Not done (deliberately):** sequential `BPI‑NNNN` references (would change the in‑flight `BidPackageEntity`; provider derives `BPI‑XXXXXXXX` from the id for now); the Bid Packages agent reading its linked mail (their Phase 2 / AI work); and auto‑tagging future replies at the moment they *arrive* (needs an on‑arrival ingestion path — the catch‑up sweep covers it at read time instead). No compiler in the authoring environment — `dotnet build` + tests are the CI gate.
