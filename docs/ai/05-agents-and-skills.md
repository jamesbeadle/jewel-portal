...
| **7.4 Timesheets Agent** | Pack: the smallest — the week-entry dialog + worker-profile read tools. Best end-to-end proof of dialog + pack + skill together. *(2026-08-28: the `submit_timesheet` this row named is gone — legacy slice deleted; the live entry is `submit_worker_week` / `SubmitWorkerWeekByName`, workers by NAME, see `01` §4.)* | 7 |
