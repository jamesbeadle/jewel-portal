...
description: >-
  ...
  Jewel_Portal MCP connector (how Claude reads and acts on the portal — there
  is no chat inside the site), and how to query the system's data directly.
---
...
| Draft the weekly Contractor's Report from a WhatsApp export folder | Read the export as before, then **match, file, draft** — `shasum -a 256` every image in the folder → `match_site_photos` once with all the hashes → look at every matched image and `archive_site_photos` the ones the `jpms-contractors-report` skill keeps out of the report (screenshots, drawings/documents, drawn-over photos, snags/defects, no-value shots — kept, restorable) → `file_site_photos` the rest per day onto that day's progress update (`create_progress_update` for a day with none) → `create_contractors_report` / `get_contractors_report`. A hash the pool does not hold is a file nobody has dropped yet: name it and ask, never re-encode or paste it. The report never names a WO number or a subcontractor's target completion date — load the portal skill `jpms-contractors-report` |
...
- The connector (Claude acting through the Jewel_Portal MCP server) sends only
  ...
  It never emails from Outlook.

## The assistant — the Jewel_Portal MCP connector

The site has **no chat of its own**: the in-portal assistant was retired on
2026-08-27 and removed. "The assistant" is Claude — desktop, Cowork or claude.ai
— talking to the portal through the Jewel_Portal MCP server (`api/Features/Mcp`,
tools in `api/Features/Ai/Tools`). It acts as the signed-in portal user: it sees
what they see, does what their buttons do, and every call is logged under their
name at `/agents/activity`. Reads mirror the pages (a feature is not done until
...
- `references/assistant-and-data.md` — the MCP connector (tool catalogue,
