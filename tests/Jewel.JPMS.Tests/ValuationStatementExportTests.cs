...
/// </summary>
public sealed class ValuationStatementExportTests
{
    [Fact]
    public void ALineMapsWithItsPreviousDerivedFromTheFrozenMovement()
    {
        ...
        var mapped = ValuationStatementExport.LineFor("Contract Works", line, _ => "Roofer");
    ...
    public void AVariationLineLeadsWithItsOwnDescription_andPaddedRef()
    {
        ...
        var mapped = ValuationStatementExport.LineFor("Variations", line, _ => null);
    ...
    public void LinesComeOutInStatementOrder()
    {
        ...
        var mapped = ValuationStatementExport.Lines(lines, _ => null);
    ...
    public void TheSummaryIsTheSnapshotsOwnFooter()
    {
        ...
        var summary = ValuationStatementExport.Summary(snapshot, lines);
    ...
    {
        var lines = new[] { Line(1, ValuationElementType.ContractWorks, "A", 1_000m, 50m, 500m, 200m) };
        var asAt = new DateTimeOffset(2026, 9, 2, 10, 0, 0, TimeSpan.Zero);

        var draft = ValuationStatementExport.Meta(new ValuationStatement(Snapshot(lines), lines, IsDraft: true, asAt));
        var frozen = ValuationStatementExport.Meta(new ValuationStatement(Snapshot(lines, locked: true), lines, IsDraft: false, asAt));

        Assert.True(draft.IsDraft);
        Assert.Equal("August 2026 — working copy", draft.StatementLabel);
        ...
        Assert.Equal("August 2026", frozen.StatementLabel);
        Assert.StartsWith("Locked 02 Sep 2026", frozen.PreparedLabel);
        Assert.Contains("issued statement", frozen.PreparedLabel);
    }
    ...
    public void TheSharedWriterProducesAWorkbookExcelOpens_withTheStatementTabs()
    {
        ...
        var workbook = ValuationReportExportWorkbook.Build(
            ValuationStatementExport.Meta(new ValuationStatement(snapshot, lines, IsDraft: true, new DateTimeOffset(2026, 9, 2, 10, 0, 0, TimeSpan.Zero))),
            ValuationStatementExport.Lines(lines, _ => null),
            ValuationStatementExport.Summary(snapshot, lines),
    ...
    // The claim carrying the statement's footer — a Draft (the working copy) or locked.
    private static ValuationClaim Snapshot(IReadOnlyList<ValuationStatementLine> lines, bool locked = false)
    {
        ...
        return new ValuationClaim(
            "CLAIM-1", "P1", 3, new DateTimeOffset(2026, 8, 31, 0, 0, 0, TimeSpan.Zero),
            locked ? ValuationClaimStatus.Preapproved : ValuationClaimStatus.Draft,
            5m, 0m, locked ? lockedAt : null, null,
            contractSum, netVariations, ValuationCalculations.RevisedContractSum(contractSum, netVariations),
            worksComplete, retention, 0m, 0m,
            ValuationCalculations.PaymentDueExVat(worksComplete, retention, 0m, 0m, 0m),
            Name: "August 2026", LockedAt: locked ? lockedAt : null);
    }
}
