...
// A form sent from a phone with no account (moved from Jeremy's forms dashboard, 2026-09-23): by its
// open address it is filed as unverified and the office is alerted without the health answers; by a
...
public sealed class PublicFormFlowTests
{
    ...
    public async Task AnOpenAddressForm_isFiledUnverified_andItsAlertLeavesTheHealthAnswersOut()
    {
        ...
        var receipt = await forms.Service.SubmitAsync(Emergency, Posted(FirstSession, EmergencyAnswers()), Address, CancellationToken.None);
        ...
        Assert.Equal(new[] { JewelBespokeBuild.Email }, alert.To);
    ...
    public async Task SentTwice_answersWithTheFirstReceipt_andFilesOnce()
    {
        ...
        var first = await forms.Service.SubmitAsync(Emergency, Posted(FirstSession, EmergencyAnswers()), Address, CancellationToken.None);
        var retry = await forms.Service.SubmitAsync(Emergency, Posted(FirstSession, EmergencyAnswers()), Address, CancellationToken.None);
    ...
    public async Task AMissingAnswer_isRefused_andNothingIsFiledOrSent()
    {
        ...
        var refusal = await Assert.ThrowsAsync<PublicFormRefusal>(() =>
            forms.Service.SubmitAsync(Emergency, Posted(FirstSession, answers), Address, CancellationToken.None));
    ...
    {
        await using var forms = new PublicFormFixture();
        var invite = new SendFormInvite(Emergency, "Sam Smith", PersonEmail, "", 7, "", "office@jewelbb.co.uk", "Jeremy");
        ...
        var opened = await forms.Service.OpenAsync(Emergency, token, null, CancellationToken.None);
        var receipt = await forms.Service.SubmitAsync(Emergency, Posted(FirstSession, EmergencyAnswers(), token), Address, CancellationToken.None);
        ...
        var again = await Assert.ThrowsAsync<PublicFormRefusal>(() =>
            forms.Service.SubmitAsync(Emergency, Posted(SecondSession, EmergencyAnswers(), token), Address, CancellationToken.None));
        Assert.Equal($"{used.Heading}. {used.Body}", again.Message);
        var reopened = await forms.Service.OpenAsync(Emergency, token, null, CancellationToken.None);
    ...
    public async Task ASignature_arrivesBeforeItsForm_inTheRestrictedStore_andTheAlertCarriesNoAnswers()
    {
        ...
        var receipt = await forms.Service.SubmitAsync(FormSlugs.RightToWork, posted, Address, CancellationToken.None);
    ...
    public async Task AFile_isOnlyTakenByAQuestionThatAsksForOne()
    {
        ...
        var refusal = await Assert.ThrowsAsync<PublicFormRefusal>(() =>
            forms.Service.UploadAsync(FormSlugs.RightToWork, upload, Address, CancellationToken.None));
