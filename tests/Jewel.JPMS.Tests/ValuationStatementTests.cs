...
// A locked valuation's statement is a value copy of the report at lock time — its summary must
// be derivable from its own lines with the same ValuationCalculations maths the live report
// uses, so the viewer, the lock, and the spreadsheet all agree.
public sealed class ValuationStatementTests
{
