...
public sealed class GraphMailClient : IGraphMailClient
{
    ...
    // drafts — mirrors the same removal in the API-side MailboxGraphClient (both clients must
    // agree or the same document would go out differently depending on whether a person or the
    // worker drafted it). A Cc'd copy arrives back in the Inbox untagged and lands straight back
    // in the triage queue; Sent Items + whole-mailbox tag reads keep the outbound leg visible.
