...
// The background workers (timer + queue triggers) live here, in a standalone Azure Function App.
// They cannot run inside the Static Web Apps managed Functions API, which only supports HTTP
// triggers. Since 2026-09-18 none of them touch the mailbox: the last one that did staged a
// request document as a draft, which was a second copy of the portal's outbound sequence, and
// that resend now goes out through the API's own send door.
//
