...
public sealed class HttpRequestRegister : IRequestRegister
{
    ...
    {
        await commands.SendAsync(new RaiseRequest(record.ProjectId, record.Kind, record.Reference, record.Title, record.Description, record.Value, record.RaisedByEmail, record.DrawingRef, record.ResponseDue, record.InternalNotes, record.ClientNotes), CancellationToken.None);
        await readModel.RefreshAsync(record.ProjectId, CancellationToken.None);
    }

    private async Task UpdateRecordAsync(Request record)
    {
        await commands.SendAsync(new UpdateRequestDetails(record.RequestId, record.Reference, record.Title, record.Description, record.Status, record.Value, record.ResponseText, record.RespondedByEmail, record.ImpliesVariation, record.DrawingRef, record.ResponseDue, record.RelatedDrawingSpec, record.InternalNotes, record.ClientNotes), CancellationToken.None);
        await readModel.RefreshAsync(record.ProjectId, CancellationToken.None);
    }
}
