...
// To; original To + Cc in Cc, minus the projects mailbox); a new email starts blank.
public partial class MailReplyComposer
{
    ...
    private string HeaderLabel =>
        StageMode ? "Reply — lined up in the Outbox, sent by Apply"
        : IsReply ? "Reply — sends from the projects mailbox"
    ...
    private string DefaultFooterNote =>
        StageMode
            ? "Nothing sends yet — the reply waits in the Outbox for this triage's Apply."
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            ...
            if (Draft is null && ReplyTo is { } replyTo) PrefillReplyEnvelope(replyTo, anchorDetail);
        }
        catch
        {
            // The live detail also feeds the reply-all Cc set; without it the reply still works,
            // addressed to the sender alone.
            if (Draft is null && ReplyTo is { } replyTo)
            {
    ...
    private async Task SendAsync()
    {
        ...
        try
        {
            var command = new SendMailboxEmail(
                ...
                MarkThreadHandled: MarkThreadHandled,
